        ...
        const post = await Post.create({
          ...
          if(hashtags){
            const result = await Promise.all(hashtags.map((tag)=> Hashtag.findOrCreate({
                ...
                await post.addHashtags(result.map((v) => v[0])); //위와같이 나오니배열로넣어줌
          }
          if(req.body.image){
              if(Array.isArray(req.body.image)){ //이미지를 여러개 올리면 image : [12.png, 23.png] isArray는 데이터가 배열인지 아닌지 체크하는것
                const images = await Promise.all(req.body.image.map((image) =>Image.create({ src : image })));
                await post.addImages(images);    
            }else{ //이미지 하나만올리는경우 배열이아님 12.png
